...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, self_attention_mask):
        ...
        ffn_outputs = self.pos_ffn(self_attention_outputs)
        ffn_outputs = self.layer_norm2(self_attention_outputs + ffn_outputs)
        # (batch_size, n_dec_seq, hidden_dim), (batch_size, num_head, n_dec_seq, n_dec_seq), (batch_size, num_head, n_dec_seq, n_enc_seq)
        return ffn_outputs, self_attention_prob
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs):
        ...
        for layer in self.layers:
            ...
            dec_outputs, self_attention_prob = layer(dec_outputs, dec_self_attention_mask,)
            self_attention_probs.append(self_attention_prob)
        # (batch_size, n_dec_seq, hidden_dim), [(batch_size, n_dec_seq, n_dec_seq)], [(batch_size, n_dec_seq, n_enc_seq)]
        return dec_outputs, self_attention_probs
...
class GPT(nn.Module):
    ...
    def forward(self, dec_inputs):
        dec_outputs, dec_self_attention_probs = self.decoder(dec_inputs)
        return dec_outputs, dec_self_attention_probs
...
class GPTPretrain(nn.Module):
    def __init__(self, config):
        ...
        self.projection_lm = nn.Linear(self.config.hidden_dim, self.config.n_dec_vocab, bias=False)
        self.projection_lm.weight = self.gpt.decoder.dec_emb.weight

    def forward(self, dec_inputs):
        dec_outputs, dec_self_attention_probs = self.gpt(dec_inputs)
        logits_lm = self.projection_lm(dec_outputs)
        return logits_lm[:, :-1, :].contiguous(), dec_self_attention_probs
